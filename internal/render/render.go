...
func CreateTemplateCache() (map[string]*template.Template, error) {
	...
	for _, page := range pages {
		...
		matches, err := filepath.Glob(fmt.Sprintf("%s/layouts/*.layouts.gohtml", pathToTemplates))
		if err != nil {
			return myCache, err
		}

		if len(matches) > 0 {
			ts, err = ts.ParseGlob(fmt.Sprintf("%s/layouts/*.layouts.gohtml", pathToTemplates))
			if err != nil {
				return myCache, err
			}
		}
